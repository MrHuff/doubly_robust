...
def get_perm(s,n,m,X_in,Y_in,Z):
    if not os.path.exists(f'real_cpu/twins_{m}'):
        os.makedirs(f'real_cpu/twins_{m}')
    if not os.path.exists(f'real_cpu/twins_{m}_null'):
        os.makedirs(f'real_cpu/twins_{m}_null')
    np.random.seed(s)
    perm_vec = np.random.permutation(n)[:m]
    T= X_in[perm_vec][:,np.newaxis]
    Y=Y_in[perm_vec][:,np.newaxis]
    X=Z[perm_vec,:]
    with open(f'real_cpu/twins_{m}/job_{s}.pickle', 'wb') as handle:
        pickle.dump({'seed': s, 'T': T, 'Y': Y, 'X': X, 'W': T}, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'real_cpu/twins_{m}_null/job_{s}.pickle', 'wb') as handle:
...
if __name__ == '__main__':
    ...
    with open(f'real_cpu/twins_cat_col_data.pickle', 'wb') as handle:
