...
def baseline_run(b_list,nn_params):
    ...
    for b in b_list:
        ...
        experiment_params={
            'experiment_save_path':f'baseline/easier_unit_baseline_{b}' , 'data_dir_load':f'easier_unit_test_b={b}', 'num_exp':100, 'nn_params':nn_params, 'training_params':training_params, 'cat_cols':[],
            'test_type':'baseline', 'debug_mode':True
        }
...
def dr_run(b_list,nn_params):
    ...
    for b in b_list:
        ...
        training_params['oracle_weights']=True
        training_params['epochs']=100
        experiment_params={
            'experiment_save_path':f'dr_exp/easier_derangement_doubly_robust_{b}' , 'data_dir_load':f'easier_unit_test_b={b}', 'num_exp':100, 'nn_params':nn_params, 'training_params':training_params, 'cat_cols':[],
            'test_type':'doubly_robust', 'debug_mode':False
        }
        c=experiment_object(**experiment_params)
        c.run_experiments()

        training_params['oracle_weights']=False
        training_params['neural_cme']=True
        training_params['epochs']=100
        experiment_params={
            'experiment_save_path':f'dr_exp/easier_prop_neural_doubly_robust_{b}' , 'data_dir_load':f'easier_unit_test_b={b}', 'num_exp':100, 'nn_params':nn_params, 'training_params':training_params, 'cat_cols':[],
            'test_type':'doubly_robust', 'debug_mode':False
        }
        c=experiment_object(**experiment_params)
        c.run_experiments()
