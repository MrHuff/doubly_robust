...
class DirectEstimator(Estimator):
    ...
    def estimate(self, sim_data):
        ...
        numpy_input = {'context_vec': np.stack(null_context_vec.values),
                       'reco_vec': np.stack(null_reco_vec.values)}
        train_input_fn = tf.estimator.inputs.numpy_input_fn(numpy_input, null_reward.values,
                                                            batch_size=self.params[1], num_epochs=self.params[2],
                                                            shuffle=True)
        classifier.train(input_fn=train_input_fn)

        numpy_input = {'context_vec': np.stack(target_context_vec.values),
                       'reco_vec': np.stack(target_reco_vec.values)}
...
class DoublyRobustEstimator(IPSEstimator):
    ...
    def estimate(self, sim_data):
        ...
        null_numpy_input = {'context_vec': np.stack(null_context_vec.values),
                            'reco_vec': np.stack(null_reco_vec.values)}
        train_input_fn = tf.estimator.inputs.numpy_input_fn(null_numpy_input, null_reward.values,
                                                            batch_size=self.params[1], num_epochs=self.params[2],
                                                            shuffle=True)
        classifier.train(input_fn=train_input_fn)

        target_numpy_input = {'context_vec': np.stack(target_context_vec.values),
                              'reco_vec': np.stack(target_reco_vec.values)}
